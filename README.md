...
- 新建、打开和切换多个独立论文库。
- 真正的一级首页可按标签、期刊或会议订阅近期论文，每次自由设置 2–10 篇，支持综合、最新、引用量和相关性排序，并可一键加入指定论文库。
...
1. 在一级“首页”点击热门研究方向即可直接切换并刷新；“订阅设置”只用于精细修改标签、期刊、会议和推荐篇数。启动时会后台刷新当天推荐，也可手动刷新；连续切换方向时只执行最后选择的刷新，不会占用 Agent 对话任务。
   窗口高度较小时，可使用首页右侧滚动条或鼠标滚轮查看推荐表、摘要和全部三个论文操作按钮；推荐表过宽时可使用表格底部横向滚动条。
2. 点击“新建论文库”，输入当前课题名称。
...
首页订阅使用 OpenAlex 的公开元数据。程序会先扩大候选范围，再按订阅条件去重和筛选，用户可设置每次展示 2–10 篇；不会把“目标 6 篇”误当成“只检索 6 篇”。列表同时显示真实发表日期、期刊/会议、OpenAlex 引用数和公开全文状态。综合排序分别计算主题命中、新近程度和候选集内的引用得分，不把三者混成一个无法解释的数字；也可以单独按最新、引用量或相关性排序。

程序把“查看论文页面”和“导入公开全文”分开处理：刷新推荐时会检查外部页面是否仍可访问，已经删除或返回 410 的页面不会继续显示为可导入；查看时回退到稳定的 OpenAlex 记录。常见中文方向会透明增加英文检索词，例如“多模态学习”同时检索 `multimodal learning`，避免只召回少量中文仓库记录。零引用、没有直接 PDF 的 Zenodo 元数据记录不会作为每日推荐候选。
...
当前“每日”含义是：应用启动时自动刷新当天结果，也可以手动刷新；没有在应用关闭后继续运行的后台定时任务和系统通知。若近期窗口内可访问的论文不足设定数量，会从同一年度候选池补足，并在列表保留真实日期，用户仍可看出哪些论文超出原窗口。
