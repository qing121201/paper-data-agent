...
from .presentation_workspace import load_spec, render_preview, revise_slide
...
class PresentationPanel:
    def __init__(self, app, parent):
        ...
        ttk.Label(parent, text="例如：加入本论文的方法原图，左文右图；或换成实验对比图并解释结论。修改会保留旧版本。",
                  wraplength=850).pack(anchor="w", pady=(4, 0))

    def refresh(self, select=None):
        if self.app.busy:
            return
        library = self.app.current_library
        self.paths = sorted((library.path / "outputs").glob("*/presentation_spec.json"),
                            key=lambda p: p.stat().st_mtime, reverse=True) if library else []
    ...
    def draw(self):
        ...
        width, height = max(20, self.preview.winfo_width() - 20), max(20, self.preview.winfo_height() - 20)
