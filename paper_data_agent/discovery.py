...
class DiscoverySubscription:
    ...
    def normalized(self) -> "DiscoverySubscription":
        return DiscoverySubscription(
            ...
            count=min(max(int(self.count), 3), 5),
...
class DiscoveryService:
    ...
    def recommend(self, subscription: DiscoverySubscription, sort_mode: str = SORT_COMBINED) -> list[DiscoveryPaper]:
        ...
        if subscription.topics and subscription.venues:
            queries = [f"{topic} {venue}" for topic in subscription.topics[:2]
                       for venue in subscription.venues[:2]]
            queries.extend(subscription.topics)
        elif subscription.topics:
            queries = list(subscription.topics)
        else:
        ...
        if len(papers) < subscription.count and subscription.recency_days < 365:
            ...
            papers.extend(item for item in self._score(raw_papers, expanded) if item.paper_key not in seen)
        key = {
        ...
        return papers[:subscription.count]
    ...
    def _score(raw_papers: list[dict[str, Any]], subscription: DiscoverySubscription) -> list[DiscoveryPaper]:
        ...
        for item, citations, relevance, freshness, citation_score, combined in candidates:
            ...
            output.append(DiscoveryPaper(
                ...
                abstract=str(item.get("abstract") or ""), doi=str(item.get("doi") or ""),
                openalex_id=str(item.get("openalex_id") or ""), public_url=urls[0] if urls else "",
                public_urls=list(dict.fromkeys(urls)), matched_queries=list(item.get("matched_queries") or []),
