"""Versioned slide specifications, previews, and evidence-aware page revision."""
...
from .assets import asset_prompt, collect_assets, resolve_images
...
def render_preview(spec_path: Path) -> list[Path]:
    ...
    if len(set(images)) != expected:
        ...
        if result.returncode:
            ...
            raise RuntimeError("PPT 文件已保留，但 PowerPoint 预览导出失败：" + detail)
...
def revise_slide(spec_path: Path, page: int, instruction: str, client) -> Path:
    ...
    current = (
        {"title": original.get("title", ""), "subtitle": original.get("subtitle", "")}
    ...
    if page and wants_images and index_path.is_file():
        assets += collect_assets(
            PaperIndex.load(index_path),
            str(current.get("title", "")) + " " + instruction,
            asset_dir,
            limit=6,
        )
    ...
    proposal_path = proposal_dir / (datetime.now().strftime("%Y%m%d-%H%M%S-%f") + ".json")
    ...
    image_actions = ("加图", "加入", "插入", "换图", "换成")
    if page and any(word in instruction for word in image_actions) and wants_images and not replacement.get("images"):
    ...
    (new_spec.parent / "revision.json").write_text(
