    ...
    for work in data.get("results", []):
        ...
        for location in [work.get("best_oa_location"), work.get("primary_location"), *(work.get("locations") or [])]:
            ...
            for candidate in (location.get("pdf_url"), location.get("landing_page_url")):
                if candidate and candidate not in public_urls:
                    public_urls.append(candidate)
        results.append({
            ...
            # Keep the public full-text target so the standalone Agent can turn
            # discovery into a real library import instead of stopping at metadata.
