...
async def render_mindmap(
    ...
    async with async_playwright() as p:
        # Installed browser channels survive Playwright cache/version changes.
        # Use them only when the matching bundled executable is missing.
        try:
            ...
            if browser is None:
                raise RuntimeError(
                    "导图内容已保存，但缺少可用的导出浏览器。请在此 Python 环境运行 "
                    "python -m playwright install chromium 后重试导出。"
                ) from exc
