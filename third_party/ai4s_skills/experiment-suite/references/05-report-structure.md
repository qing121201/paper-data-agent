...
If a displayed equation is necessary, follow it immediately with a “where …”
sentence defining every symbol, operator, and index not defined earlier in the
report. Define each symbol once at first use.

### 5. Results

The substantive section. Two passes:

**5.a — Headline numbers.** A markdown table summarising the main comparison. Format (shown in the default *simulated* mode):
...
**Bolding the winning row is conditional on provenance** (`results.json` provenance mode):

- **Simulated (default):** leave it unbolded, as above. The numbers were generated by the agent, so bolding `Ours` as the best row asserts an empirical victory that never happened — the "simulated" caption discloses the shape, but a bolded win reads as a real result at a glance. Let the disclosure and the plain numbers speak.
- **Measured:** bold `**Ours**` (and the best cell per column) only when it genuinely holds on the user's measured numbers — this is the normal measured-mode convention and is honest there.
...
- [ ] Headline table present; winning row bolded **only in measured mode** (unbolded when simulated), "Numbers are simulated" caption when simulated
